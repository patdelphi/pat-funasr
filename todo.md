...
完成标准：顶层仍只有“模型与服务”，内部按现有五个功能区形成二级 Tab；不复制组件、接口或状态逻辑，父 Tab 不绑定阻塞性后端事件，五个子页可独立进入并显示原功能。

- [x] 增加导航结构测试，约束五个子 Tab 的名称、顺序和按需渲染。
- [x] 把服务总览、模型管理、运行资源、任务队列、诊断与日志从 Accordion 改为二级 Tab。
- [x] 移除父级“模型与服务”的后端 select 刷新事件，子页状态切换使用非排队轻量事件。
- [x] 保留检查服务、刷新模型、资源/任务面板、日志刷新与下载的原事件和后端接口；资源与任务子页增加显式刷新按钮。
- [x] 重启服务并在浏览器逐项切换五个子页；检查服务、刷新资源和刷新任务均取得真实结果，页面保持存活。
- [x] 相关测试 `47 passed`、全量测试 `238 passed, 3 subtests passed`、compileall 和格式检查通过；规划、changelog 与 chat_history 已更新。

## 2026-08-24 实时识别切换卡死再修复

- [x] 复现并检查“进入实时识别后切换其他栏目”路径，确认服务/API 正常，问题集中在业务 Tab 后端 `select` 导航回调。
- [x] 先增加回归断言，要求所有非服务顶层/子 Tab 不得注册后端 `select` 依赖；旧实现按预期失败。
- [x] 删除 6 个非服务业务子 Tab 的 `select` 回调，保留模型与服务子页的 `queue=False` 状态回调。
- [x] 重启 CUDA 托管 API/UI；浏览器验证实时识别切换至转录工作台、媒体与文本工具、模型与服务均正常，页面无错误。
- [x] 全量测试 `234 tests OK`，compileall 通过，API `/health` 和 WebUI 首页均返回 200。
