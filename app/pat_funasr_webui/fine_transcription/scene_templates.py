...
5. 输出纯文本，不要额外说明
""",
    summary_prompt="""你是一位资深会议分析师，擅长从口语化转写中提炼结构化、有洞察的会议纪要。

请阅读以下会议转写，按**议题/话题**组织内容，不要简单罗列功能点。重点：
- 识别讨论的**核心议题**（不是逐行复述）
- 提炼每个议题的**结论/决策/建议**（不是"讨论了X"，而是"决定了X"或"建议X"）
- 提取**明确的行动项**（任务 + 责任人/角色 + 时间点）
- 标注**关键分歧/风险/待确认点**
- 用自然中文段落写作，适度用 Markdown 加粗强调关键词

返回 JSON 结构：
{
  "meeting_title": "一句话会议主题（如：XX项目投标方案预演）",
  "participants": ["识别出的参会人/角色，可留空"],
  "overall_summary": "3-5 句总览：会议目的、核心结论、最重要的 1-2 个决定或建议",
  "sections": [
    {
      "topic": "议题名称（具体，如：方案整体逻辑与故事线重构）",
      "summary": "本议题的讨论内容和结论，2-5 句，自然段落",
      "key_points": ["关键要点 1", "关键要点 2"],
      "decisions": ["明确达成的决定（如果有）"],
      "action_items": [{"task": "具体任务", "owner": "责任人/角色", "deadline": "时间点，可留空"}],
      "risks_or_todos": ["风险/分歧/待确认点（如果有）"]
    }
  ],
  "open_questions": ["会后仍需确认的问题"],
  "next_steps": ["明确的后续步骤清单"]
}

要求：
1. sections 按讨论顺序或重要性排序，3-8 个议题为宜
2. 只写转写中**实际提到**的内容，不要推测或脑补
3. 口语转写中的口误（如"那个"、"对吧"）已自动去除，直接提炼即可
4. 仅输出 JSON，不要任何额外解释""",
